...
class _PiChatViewState extends State<PiChatView> {
  ...
  void _connectWebSocket() {
    ...
    _messages.add(
      ChatMessage(
        role: "system",
        text: "Connected to Pi Agent. Ready to assist.",
      ),
    );
  }
  ...
  void _onMessage(dynamic message) {
    ...
    setState(() {
      if (type == 'agent_event' && data['node'] == 'pi') {
        _messages.add(
          ChatMessage(role: "assistant", text: data['message'] ?? ''),
        );
        _scrollToBottom();
        ...
        _messages.add(
          ChatMessage(
            role: "system",
            text: "⚠️ HITL Approval Required: ${data['command']}",
          ),
        );
        _showApprovalDialog(
  ...
  void _onError(dynamic error) {
    setState(
      () => _messages.add(
        ChatMessage(role: "system", text: "Connection error: $error"),
      ),
    );
  }
  ...
  void _onDone() {
    setState(
      () => _messages.add(
        ChatMessage(role: "system", text: "Disconnected. Reconnecting..."),
      ),
    );
  ...
  void _sendMessage() {
    ...
    _channel?.sink.add(jsonEncode({"type": "pi_chat", "message": text}));
    _scrollToBottom();
  }
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: isUser
                          ...
                          : (isSystem ? Colors.black45 : Colors.grey.shade800),
                ...
                Expanded(
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      hintText:
                          'Ask Pi to execute local tasks, manage clipboard, etc...',
...
class _TeamOrchestratorViewState extends State<TeamOrchestratorView> {
  ...
  final TextEditingController _companyDescController =
      TextEditingController(); // Added for dynamic setup
  WebSocketChannel? _channel;
  ...
  final String _clientId =
      "team_client_${DateTime.now().millisecondsSinceEpoch}";
  ...
  Future<void> _fetchRESTData() async {
    try {
      ...
      final teamRes = await http.get(
        Uri.parse('http://localhost:8001/api/teams'),
      );
        ...
        setState(() {
          ...
          if (_teamPresets.isEmpty) {
            ...
            _teamPresets = [
              {
                "id": "default",
                "name": "Default Pair (Dev+Reviewer)",
                "config": [
                  {"name": "developer"},
                ],
              },
  ...
  void _onMessage(dynamic message) {
    ...
    setState(() {
      ...
      if (type == 'info') {
        ...
        if (_generatedOrgChart != null &&
            _generatedOrgChart!['departments'] != null) {
          List<dynamic> depts = _generatedOrgChart!['departments'];
          List<Map<String, String>> newConfig = [];
          for (var d in depts) {
            if (d['default_enabled'] == true &&
                d['agents'] != null &&
                (d['agents'] as List).isNotEmpty) {
  ...
  void _requestAiOrgChart() {
    ...
    if (_selectedProjectId == null) {
      _logs.add(
        "[WARNING] Please select or create a project first so AI knows where to save the manuals.",
      );
      return;
    }
  ...
  void _deployTeam() {
    ...
    List<dynamic> targetConfig = [
      {"name": "developer"},
    ]; // fallback
    if (_selectedTeamId != null) {
      final team = _teamPresets.firstWhere(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  decoration: const InputDecoration(
                    labelText: 'Select Project context...',
                    border: OutlineInputBorder(),
                  ),
                  ...
                  items: [
                    const DropdownMenuItem(
                      value: null,
                      child: Text("Create New Temporary Project"),
                    ),
                    ..._projects.map(
                      (p) => DropdownMenuItem(
                        value: p['id'].toString(),
                ...
                TextButton.icon(
                  ...
                  label: const Text("Create Global Project"),
                ),

                const Divider(),

                Text(
                  "2. AI Org & Skills Builder",
                  style: Theme.of(context).textTheme.titleMedium,
                ),
                const SizedBox(height: 8),
                TextField(
                  ...
                  decoration: const InputDecoration(
                    labelText:
                        'Describe company target & goals (e.g. "We are an indie game studio building a 2D RPG")',
                    border: OutlineInputBorder(),
                  ),
                ),
                const SizedBox(height: 8),
        ...
        Expanded(
          ...
          child: Container(
            ...
            child: _kanbanTasks.isEmpty
                ? Center(
                    child: Text(
                      "Deploy a team to see the Kanban board.",
    ...
    return Column(
      children: [
        Container(
          ...
          child: const Row(
            children: [
              Icon(Icons.view_kanban, color: Colors.indigoAccent),
              SizedBox(width: 8),
              Text(
                'Team Kanban Board',
                  ...
                  child: Column(
                    children: [
                      Container(
                        padding: const EdgeInsets.all(8),
                        child: Text(
                          "${col['icon']} ${col['label']} (${tasks.length})",
                          style: TextStyle(
                            color: Colors.grey.shade400,
                            fontSize: 12,
                          ),
